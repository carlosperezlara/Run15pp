...
int main(int argc, char *argv[]){
  ...
  TProfile *hMeanTDC1[8];
  TProfile *hMeanTDC2[8];
  TProfile *hMeanTDC3[8];
  TProfile *hMeanTDC4[8];
  for( int t=0; t!=8; ++t ) {
    hMeanTDC1[t] = new TProfile( Form("MeanTDC1_SEC%d",t),"",
				 secs[t+1]-secs[t],secs[t]-0.5,secs[t+1]-0.5); 
    hMeanTDC2[t] = new TProfile( Form("MeanTDC2_SEC%d",t),"",
				 secs[t+1]-secs[t],secs[t]-0.5,secs[t+1]-0.5);
    hMeanTDC3[t] = new TProfile( Form("MeanTDC3_SEC%d",t),"",
				 secs[t+1]-secs[t],secs[t]-0.5,secs[t+1]-0.5);
    hMeanTDC4[t] = new TProfile( Form("MeanTDC4_SEC%d",t),"",
				 secs[t+1]-secs[t],secs[t]-0.5,secs[t+1]-0.5);
 }
  ...
  for(Long64_t i1=0; i1<nevt; ++i1) {
    ...
    for(unsigned int i=0; i!=nnn ;++i) {
      ...
      if(adc>1000&&adc<2000) {
	hMeanTDC1[isc]->Fill( twid, tdc );
      }
      if(adc>2500&&adc<4000) {
	hMeanTDC2[isc]->Fill( twid, tdc );
      }
      if(adc>2000&&adc<2100) {
	hMeanTDC3[isc]->Fill( twid, tdc );
      }
      if(adc>380&&adc<480) {
	hMeanTDC4[isc]->Fill( twid, tdc );
      }
    }
  }
  hEvents->SetBinContent(1,goodevents);
  TString outfile = Form("out/MyResults3_%s.root",srun.Data());
