...
int main(int argc, char *argv[]){
  ...
  for(Long64_t i1=0; i1<nevt; ++i1) {
    ...
    for(unsigned int i=0; i!=nnn ;++i) {
      ...
      float tof2=tof1 -step4_sector[isc]; // con esto calculo el offsettwr
      float tof3=tof2 -offtwr[twid]; // final!
      hADCTDC->Fill( twid, tof3 );
    }
  }
  hEvents->SetBinContent(1,goodevents);
  TString outfile = Form("out/MyResultsTOF5_%s.root",srun.Data());
